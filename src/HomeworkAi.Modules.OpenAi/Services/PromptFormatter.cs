...
public class PromptFormatter : IPromptFormatter
{
   public string FormatExerciseBaseData(ExerciseQueryBase baseData)
   {
      var prompt =
            ...
            $"3. The exercise header (with information about exercise - not the exercise.) must be in {(baseData.ExerciseHeaderInMotherLanguage ? baseData.MotherLanguage : baseData.TargetLanguage)}. The header is just basic information about the exercise. Don't confuse the language of the header with the language of the sentences generated. " +
        ...
        if (!string.IsNullOrWhiteSpace(baseData.GrammarSection))
            prompt += $"7. The exercise must be based on the grammatical element - {baseData.GrammarSection}. Remember to adjust the level of the grammatical element to the level of the exercise. Don't create sentences using other grammatical elements.\n";
        else
            prompt += $"7. No grammatical elements are imposed top-down in the exercise. However, you can emphasize them keeping in mind the level of language proficiency.\n";

        if (!string.IsNullOrWhiteSpace(baseData.SupportMaterial))
            prompt += $"8. In the support material section know these support materials: {baseData.TopicsOfSentences}.\n";
        else
            prompt += $"8. In the support material section, generate a short summary that will be useful to the student during the exercise. Don't point out the answer there, only give hints.\n";
